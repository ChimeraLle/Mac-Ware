...
std::vector<std::string> get_rizz(const std::string& path) {
	...
	try {
		...
		if (file.is_open()) {
			...
			size_t pos = contents.find(" size: ");
			if (pos != std::string::npos) {
				result.push_back(contents.substr(0, pos));
				result.push_back(contents.substr(pos + 7));
			}
			file.close();
		}
	}
	...
	svr.Post("/bridge", [](const Request& req, Response& res) {
			...
			if (json_body.contains("action") && json_body.contains("script")) {
				...
				if (action == "loadstring") {
					...
					std::vector<std::string> compiled = get_rizz("test.btc");

					std::vector<uint8_t> sigma_rizz(compiled[0].begin(), compiled[0].end());


					module_script.SetBytecode(sigma_rizz, std::stoi(compiled[1]));
